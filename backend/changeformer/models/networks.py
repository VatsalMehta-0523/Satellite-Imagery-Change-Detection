...
def define_G(args=None, net_G='ChangeFormerV6', embed_dim=256, init_type='normal', init_gain=0.02, gpu_ids=[]):
    # Support both old 'args' style and new direct parameters
    if args is not None:
        net_G = getattr(args, 'net_G', net_G)
        embed_dim = getattr(args, 'embed_dim', embed_dim)
        gpu_ids = getattr(args, 'gpu_ids', gpu_ids)
    ...
    elif net_G == 'ChangeFormerV1':
        net = ChangeFormerV1() #ChangeFormer with Transformer Encoder and Convolutional Decoder
    
    elif net_G == 'ChangeFormerV2':
        net = ChangeFormerV2() #ChangeFormer with Transformer Encoder and Convolutional Decoder

    elif net_G == 'ChangeFormerV3':
        net = ChangeFormerV3() #ChangeFormer with Transformer Encoder and Convolutional Decoder (Fuse)

    elif net_G == 'ChangeFormerV4':
        net = ChangeFormerV4() #ChangeFormer with Transformer Encoder and Convolutional Decoder (Fuse)
    
    elif net_G == 'ChangeFormerV5':
        net = ChangeFormerV5(embed_dim=embed_dim) #ChangeFormer with Transformer Encoder and Convolutional Decoder (Fuse)

    elif net_G == 'ChangeFormerV6':
        net = ChangeFormerV6(embed_dim=embed_dim) #ChangeFormer with Transformer Encoder and Convolutional Decoder (Fuse)
    
    elif net_G == "SiamUnet_diff":
